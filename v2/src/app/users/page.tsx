...
async function loadGooglePlacesServices() {
  ...
  googlePlacesLoader = apiJson<{ google_maps_api_key?: string | null }>(
    ...
    if (!config.google_maps_api_key) throw new Error("Google Maps is not configured.");
    return new Promise<void>((resolve, reject) => {
      const script = document.createElement("script");
      script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(config.google_maps_api_key as string)}&libraries=places`;
      script.async = true;
      ...
      loadGooglePlacesServices()
        .then(() => {
          ...
          service.getPlacePredictions(
            { input: query, componentRestrictions: { country: "za" }, types: ["address"] },
            (results, status) => {
              if (!active) return;
        ...
        .catch((error) => {
          if (active)
            setMapsStatus(
              error instanceof Error ? error.message : "Google address search is unavailable.",
            );
        })
        .finally(() => undefined);
    }, 300);
  ...
  async function chooseSuggestion(suggestion: AddressSuggestion) {
    ...
    try {
      ...
      const result = await new Promise<GoogleGeocoderResult>((resolve, reject) => {
        new maps.Geocoder().geocode({ placeId: suggestion.place_id }, (results, status) => {
          if (status === maps.GeocoderStatus.OK && results?.[0]) resolve(results[0]);
          else reject(new Error("Unable to confirm this Google address."));
        });
      });
      ...
      onSelected(address);
      setSuggestions([]);
      setMapsStatus("Google address selected and coordinates captured.");
    } catch (error) {
      setMapsStatus(
        error instanceof Error ? error.message : "Unable to confirm this address.",
      );
    }
  }
  return (
          ...
          <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 overflow-hidden rounded-2xl border border-[var(--line)] bg-white shadow-xl">
            {loadingSuggestions && !suggestions.length ? (
              <div className="flex items-center gap-2 p-4 text-sm text-[var(--muted)]">
                <LoaderCircle size={16} className="animate-spin" /> Searching Google...
              </div>
            ) : (
              suggestions.map((suggestion) => (
                ...
                >
                  <MapPin size={16} className="mt-0.5 shrink-0 text-[var(--blue)]" />
